...
class ComplaintOperation extends ReferencesOperation
{
    ...
    const TYPE_CHANGE = 2;
        ...
        /**
        * Описаный код обрабатывает событие создания/изменении жалобы, формирует сообщения и рассылает их (email/смс)

        *Качество кода мягко говоря на низком уровне:
            *- нет обработки входных параметров от клиента
            *- не используются паттерны проектирования
            *- возврат текста ошибки и http кода через Exception
            *- множество ошибок бизнес логики
            *- ошибки в коде (нет обработки/перехвата ошибки отправки сообщений, переопределение переменой в цикле)
            *- имена некоторых методов не отражают результат работы функции (например getFullName)
            *- вся логика doOperation содержится в одном методе
            *- не защищены свойства классов
            *и так далее.
        */
